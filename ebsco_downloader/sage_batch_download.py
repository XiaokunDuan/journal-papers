...
PROGRESS_FILE = ROOT / "progress_sage.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Automate SAGE POM PDF downloads from papers.json."
    )
...
async def pick_sage_page(context: BrowserContext) -> Page:
    for page in reversed(context.pages):
        if SAGE_HOST in page.url or CNP_HOST in page.url:
            await page.bring_to_front()
            return page
    page = await context.new_page()
    await page.goto(f"https://{CNP_HOST}/", wait_until="domcontentloaded")
    await page.bring_to_front()
    return page
...
async def ensure_page_alive(context: BrowserContext, page: Page | None) -> Page:
    if page is not None and not page.is_closed():
        return page
    return await pick_sage_page(context)


async def build_cookie_header(context: BrowserContext) -> str:
    cookies = await context.cookies([f"https://{CNP_HOST}/", f"https://{SAGE_HOST}/"])
    if not cookies:
...
def download_via_http(url: str, cookie_header: str, destination: Path) -> None:
    ...
    req = Request(
        ...
        headers={
            ...
            "Referer": f"https://{CNP_HOST}/search/search",
            "User-Agent": (
    ...
    if not data.startswith(b"%PDF-"):
        ...
        raise RuntimeError(f"sage pdf link did not return PDF: content-type={content_type!r}, preview={preview!r}")
    temp_path.write_bytes(data)
    temp_path.replace(destination)


async def save_download(download: Download, destination: Path) -> None:
    destination.parent.mkdir(parents=True, exist_ok=True)
    temp_path = destination.with_name(f".{destination.name}.part")
    if temp_path.exists():
        temp_path.unlink()
...
async def open_cnp_home(page: Page) -> Page:
    for _ in range(3):
        await page.goto(f"https://{CNP_HOST}/", wait_until="domcontentloaded", timeout=45000)
        try:
            await page.wait_for_load_state("networkidle", timeout=10000)
        except PlaywrightTimeoutError:
            pass
        if await page.locator('input[placeholder="Search Journals"]').count():
            return page
        await page.wait_for_timeout(1500)
    raise RuntimeError("CNPeReading home/search box not available")


async def find_result_link_in_items(page: Page, title: str) -> str | None:
    ...
    for index in range(count):
        ...
        if not text:
            continue
        if target not in compact_text(text):
            continue
        download_links = item.locator('a[href*="/paragraph/download/"]')
        if await download_links.count():
            href = await download_links.first.get_attribute("href")
            if href:
                return href_to_absolute(href)
        article_links = item.locator('a[href*="/paragraph/article/"]')
        if await article_links.count():
            href = await article_links.first.get_attribute("href")
            if href and "/paragraph/article/" in href:
...
async def search_download_link(page: Page, title: str, doi_url: str) -> str:
    ...
    for query in build_search_queries(title):
        ...
        try:
            ...
            pass
        result_link = page.locator(f'a[href*="/paragraph/download/?doi={doi_suffix}"]').first
        if await page.locator(f'a[href*="/paragraph/download/?doi={doi_suffix}"]').count():
            href = await result_link.get_attribute("href")
            if not href:
                raise RuntimeError(f"CNPeReading download link missing href for doi={doi_suffix}")
            return href_to_absolute(href)
        fallback_href = await find_result_link_in_items(page, title)
        if fallback_href:
            return fallback_href
        last_body = (await page.locator("body").inner_text())[:1000]
    raise RuntimeError(f"no CNPeReading download link found for doi={doi_suffix}; page={last_body!r}")
...
async def process_with_retry(page: Page, paper: dict[str, Any], retries: int = 2) -> Page:
    last_exc: Exception | None = None
    for attempt in range(retries + 1):
        try:
            return await process_one_paper(page, paper)
...
async def run_mode(args: argparse.Namespace) -> None:
    ...
    async with async_playwright() as pw:
        ...
        page = await pick_sage_page(context)
        for index, paper in enumerate(todo, start=1):
            ...
            try:
                ...
                page = await process_with_retry(page, paper)
                mark_done(progress_file, progress, paper)
                print(f"  saved -> {destination}")
